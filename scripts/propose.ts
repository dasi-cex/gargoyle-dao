...
async function main() {
  ...
  const proposalTx = await await governor.propose(
  ...
  const proposalLogs = receipt?.logs as TypedEventLog<any>[];
  const proposalCreatedEvent = proposalLogs?.find(x => x.fragment.name === 'ProposalCreated');
